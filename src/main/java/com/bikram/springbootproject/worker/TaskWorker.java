...
//    }
    ...
    public void handlemailDLQ(TaskMessage taskMessage){
        markFailedFromDlq(taskMessage, RabbitMQConfig.EMAIL_DLQ);
    }
    @RabbitListener(queues = RabbitMQConfig.CSV_IMPORT_DLQ)
    public void handleCsvDLQ(TaskMessage taskMessage){
        markFailedFromDlq(taskMessage, RabbitMQConfig.CSV_IMPORT_DLQ);
    }
    @RabbitListener(queues = RabbitMQConfig.REPORT_DLQ)
    public void handleReportDLQ(TaskMessage taskMessage){
        markFailedFromDlq(taskMessage, RabbitMQConfig.REPORT_DLQ);
    }
    @RabbitListener(queues = RabbitMQConfig.OTHERS_DLQ)
    public void handleOthersDLQ(TaskMessage taskMessage){
        markFailedFromDlq(taskMessage, RabbitMQConfig.OTHERS_DLQ);
    }
    private void markFailedFromDlq(TaskMessage message, String dlqName) {
        log.error("[DLQ] Task [{}] landed in {} after exhausting retries", message.getTaskId(), dlqName);

        taskRepo.findById(message.getTaskId()).ifPresentOrElse(task -> {
            if (task.getStatus() != TaskStatus.COMPLETED && task.getStatus() != TaskStatus.CANCELED) {
                task.setStatus(TaskStatus.FAILED);
                task.setError("Task failed after exhausting all AMQP retries. Check DLQ: " + dlqName);
                taskRepo.save(task);
                log.error("[DLQ] Task [{}] marked FAILED in DB", task.getId());
            }
        }, () -> log.warn("[DLQ] Task [{}] not found in DB", message.getTaskId()));
    }
}
